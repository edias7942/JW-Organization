...
export default function TerritoriesTable() {
  ...
  useEffect(() => {
    Axios.post("http://localhost:3001/territories").then((response) =>
  ...
  function openTerritory(element) {
    ...
    let openedTerritoryContainer = document.getElementById(
      "opened-territory-container"
    );
    ...
    <div id="container-territories">
          ...
          territoriesList.map((territory) => {
            return (
              <div
                ...
                id={`territory-card-${territory.NUMBER}`}
                className="territory-card"
                onClick={() => openTerritory(territory)}
              >
                <img
                  territory={territory.NUMBER}
                  className="territory-img"
                  src={territoriesImages[territory.ID]}
                  alt=""
                />
                <div
                  territory={territory.NUMBER}
                  className="territory-section1"
                >
                  <div
                    territory={territory.NUMBER}
                  ...
                  >
                    <div
                      territory={territory.NUMBER}
                      className="territory-title-title"
                    >
                      {" "}
                    ...
                    <div
                      territory={territory.NUMBER}
                      className="territory-localization"
                    >
                  ...
                  <div
                    territory={territory.NUMBER}
                    title={capitalize(territory.STATUS)}
                    className={"territory-status " + territory.STATUS}
                  >
                    {capitalize(territory.STATUS)}
                  </div>
                </div>
              </div>
            );
      ...
      <div id="opened-territory-container">
        <div id="ot-header">
          ...
          />
          <div id="ot-header-title">
            Território {currentTerritory.NUMBER} :{" "}
            {currentTerritory.LOCALIZATION}
          </div>
          <img id="ot-header-back" src={ArrowLeftIcon} alt="" />
        </div>
        <div id="ot-images">
          <img
            id="ot-img"
            src={territoriesImages[currentTerritory.ID]}
            alt=""
          />
          <div id="ot-streets">
            ...
            {streets.map((e, i) => (
              <div id={`ot-street${++i}`} className="ot-street">
                <img
                  src={mapIcons[e.slice(1, 2)]}
                  alt=""
                  className="ot-street-icon"
                />
